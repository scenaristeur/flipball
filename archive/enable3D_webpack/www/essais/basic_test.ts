...
class MainScene extends Scene3D {
  ...
  create() {
    ...
    let pivot = this.physics.add.cylinder(
      ...
      let blockerL1 = this.physics.add.cylinder(
        {
          x:2, y: 4, z:1, height: 3, radiusBottom: 0.2, radiusTop: 0.2,
          collisionFlags: 1
        }
      )
      pivot.add(blockerL1)
      ...
      flipL = this.physics.add.box({x:2, y:2, width:6, mass:1})
  ...
  function flip(f){
    ...
    f.body.applyImpulse({x:0, y: 0, z: -40}, {x:f.body.position.x+2, y:f.body.position.y, z: f.body.position.z})
    console.log(f.body.angularVelocity)
...
//   console.log(f.body.position)
  f.body.applyImpulse({x:3, y: 0, z: 10}, {x:f.body.position.x+2, y:0, z: 0})
