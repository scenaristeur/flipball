...
export class MainScene extends Scene3D {
  ...
  create() {
    ...
    window.addEventListener(
      "configChanged",
      function (e) {
        console.log("configChanged", e.detail);
        rotationSpeed = parseFloat(e.detail.rotationSpeed);
    ...
    window.addEventListener(
      ...
      async function (e) {
        ...
        try {
          scene.warpSpeed();
          scene.config = await import(
            "@/tables/" + scene.tablename + "/config.json"
          );
          ...
          if (scene.config.type == "marble") {
            ...
            for (let x = "-10"; x < 11; x++) {
              ...
              for (let z = "-10"; z < 11; z++) {
                ...
                let capteur = scene.physics.add.box(
                  { x: x, y: 1, z: z, collisionFlags: 1 },
                ...
                capteur.body.on.collision((otherObject, event) => {
                  if (
                    otherObject.name !== "ground" &&
                    otherObject.type != "capteur"
                  ) {
          ...
          alert(
            "I can't open '" +
              scene.tablename +
              "/config.json' in the tables folder !"
          );
        }
      },
      false
    ...
    for (let p of parts) {
      ...
      loader.load(
        ...
        function (geometry) {
          ...
          object.add(part);
          p.scale != undefined
            ? object.scale.set(p.scale.x, p.scale.y, p.scale.z)
            : "";
          p.position != undefined
            ? object.position.set(p.position.x, p.position.y, p.position.z)
            : "";
          p.rotation != undefined
            ? object.rotation.set(p.rotation.x, p.rotation.y, p.rotation.z)
            : "";
